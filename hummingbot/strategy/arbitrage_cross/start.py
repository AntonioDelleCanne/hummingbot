...
from hummingbot.strategy.arbitrage_cross.arbitrage_cross import ArbitrageCrossStrategy
from hummingbot.strategy.arbitrage_cross.arbitrage_cross_config_map import arbitrage_cross_config_map


def start(self):
    primary_market = arbitrage_cross_config_map.get("primary_market").value.lower()
    secondary_market = arbitrage_cross_config_map.get("secondary_market").value.lower()
    raw_primary_trading_pair = arbitrage_cross_config_map.get("primary_market_trading_pair").value
    raw_secondary_trading_pair = arbitrage_cross_config_map.get("secondary_market_trading_pair").value
    min_profitability = arbitrage_cross_config_map.get("min_profitability").value / Decimal("100")
    secondary_to_primary_base_conversion_rate = arbitrage_cross_config_map["secondary_to_primary_base_conversion_rate"].value
    secondary_to_primary_quote_conversion_rate = arbitrage_cross_config_map["secondary_to_primary_quote_conversion_rate"].value
    ...
    self.market_pair = ArbitrageCrossMarketPair(*self.market_trading_pair_tuples)
    self.strategy = ArbitrageCrossStrategy(market_pairs=[self.market_pair],
                                      min_profitability=min_profitability,
                                      logging_options=ArbitrageCrossStrategy.OPTION_LOG_ALL,
